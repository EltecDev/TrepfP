...
public class GetRealDataFromHexaImberaHealth {
    ...
    public static List<String> convert(List<String> arrayLists, String action){
        ...
        switch (action){
            ...
            case "Lectura de parámetros de operación":{
                ...
                //guardar los valores de CD y CE
                arrayListInfo.add(s.substring(206,208));
                arrayListInfo.add(s.substring(208,210));
            ...
            case "Lectura de datos tipo Tiempo real":{
                if (!arrayLists.isEmpty()){
                    StringBuilder s = cleanSpace(arrayLists);
                    String isChecksumOk = checkChecksum(s.toString());
                    //if (isChecksumOk.equals("ok")){
                        ...
                        //arrayListInfo.add(s.substring(36,40));//tempDisplay
                        //arrayListInfo.add(s.substring(40,42));//estados
            ...
            case "Lectura de estados":{
                if (!arrayLists.isEmpty()){
                    StringBuilder s = cleanSpace(arrayLists);
                        Log.d("Lectura de estadosGETTT",":"+s);
                        ...
                        //String buffer = String.valueOf(getDecimal(s.substring(14,16)));//indica el tamaño del buffer de datos para saber si contiene la tempDisplay

                        arrayListInfo.add(s.substring(16,18));//registro 1
        ...
        switch (action){
            case "Handshake":{
                ...
                if(data.isEmpty()){
                    ...
                    newData.add(getFwVersionFromHex(data.get(3).substring(0,2), data.get(3).substring(2)));
            ...
            case "Lectura de parámetros de operación":{
                ...
                if(data.isEmpty()){
                    ...
                    //header
                    Log.d(TAG, "GetRealData: "+data.size());
                    StringBuilder v = new StringBuilder();
                    v.append(getDecimal(data.get(0).substring(0,2)));
                    v.append(".");
                    v.append(getDecimal(data.get(0).substring(2)));

                    newData.add(v.toString());
                    ...
                    int i = 4;
                    do {
                        if (i==22 || i==23|| i==24 ||i==25 || i == 26 || i == 27 ||i==28 || i == 29|| i == 30 || i == 31|| i == 32||i == 33 ||i == 34||i == 35|| i==38 || i==40||  i==54|| i==55|| i==56|| i==57 ){
                            newData.add(String.valueOf(getDecimal(data.get(i))));//decimales sin punto
                        }
                        else {
                            if (i == 59 || i == 60) {//MODELOS
                                newData.add(hexToAscii(data.get(i)));//a ASCII
                            }else if(i==61||i==66||i==67){//PARAM ID
                                newData.add(data.get(i));
                            }else if (i==4||  i==6 || i==7 || i==8 || i==9 || i==10 || i==13 || i==14 || i==15  || i==16 || i==17 || i==20 || i==21 ){ //positivos y negativos de -99.0 a 99.0
                                ...
                                //Extraccion opciones segùn bit usado
                                newData.add(getOptionSwitch(data.get(i),"banderasConfig2"));
                            }else{
            ...
            case "Lectura de estados": {
                ...
                if(data.isEmpty()){
                    ...
                    newData.add(getEstados(data.get(4)));
                }
                return newData;
            }
            case "Lectura de datos tipo Tiempo real": {
                ...
                if(data.isEmpty()){
                    //newData.add(getSameData(data.get(0),"trefpversion"));
                    newData.add("nullRealtimeStatus");
                }else {
                    //header
                    /*
                    newData.add(getSameData(data.get(0), "trefpversion"));
                    ...
                    //if (buffer.equals("16")){
                        ...
                        //temp display
                        /*numf = getDecimalFloat(data.get(10));
                        num = (int) numf;
                        if (num<99.99){
                            newData.add(String.valueOf(getDecimalFloat(data.get(10)))); //decimales con punto //get temp positivo
                        }else if (num>99.99){
                            newData.add(getNegativeTempfloat("FFFF"+data.get(10))); //get negativos
                        }else{//Es 0 cero
                            newData.add("0000"); //get negativos
                        }*/
    ...
    public static List<String> convertPlantillaRemoto(String s){//todo ESTÀ HECHO EN OTRO MÈTODO PORQUE EN trefp pc SE MANDA CON CHECKSUM MAS PEQUEÑOS, REVISAR SI ES NECESARIO CAMBIAR EN PC
        ...
        arrayListInfo.add("00");//software version
        arrayListInfo.add("00");//buffer_size
        arrayListInfo.add("00");//data_type
        arrayListInfo.add("00");//data_size
        do {
            if (i==30){//saltar posiciones por parámetros que no se están usando
                i = i+12;
            }else if (i==50){
        ...
        do {
            if (i==164){//saltar posiciones por parámetros que no se están usando
                i = i+2;
            }else if (i==172){//saltar posiciones por parámetros que no se están usando
                i = i+4;
            }else if (i==192){
                i = i+2;
            }else if (i==198){//nuevo cambio, se quitar C9 se agrega color CD
                i = i+8;
            }else if (i==208){
                i = i+4;
            }else if (i==224){
    ...
    public static List<String> GetRealDataPLantillaRemoto(List<String> data){
        ...
        if(data.isEmpty()){
            ...
            int i = 4;
            do {
                if (i==20 ||i==21 ||i==22 || i==23|| i==24 ||i==25 || i == 26 || i == 27 ||i==28 || i == 29|| i == 30 || i == 31|| i == 32|| i==35 || i==37|| i == 41|| i == 50 || i==51|| i==52|| i==53 ){
                    newData.add(String.valueOf(getDecimal(data.get(i))));//decimales sin punto
                }else {
                    if (i == 55 || i == 56) {//MODELOS
                        newData.add(hexToAscii(data.get(i)));//a ASCII
                    }else if(i==57){//PARAM ID
                        newData.add(data.get(i));
                    }else if (i==4||  i==6 || i==7 || i==8 || i==9 || i==10 || i==13 || i==14 || i==15  || i==16 || i==17 ){ //positivos y negativos de -99.0 a 99.0
                        float j = getDecimalFloat(data.get(i)); // decimales con punto
                        if (j>99.9){
                            //Extraccion de temperaturas en decimales
                            newData.add(getNegativeTempfloat("FFFF"+data.get(i)));
    ...
    private static String getEstados(String s){
        String ss = HexToBinary(s);
        StringBuilder stringBuilder = new StringBuilder();
        String c;
        Log.d("DATOBINARIO",":"+ss);
        c = ss.substring(7,8);//Salida nocturno
        if(c.equals("1"))
            stringBuilder.append("1");
        else
            stringBuilder.append("0");

        c = ss.substring(6,7);//estado de ONOFF
        if(c.equals("1"))
            stringBuilder.append("1");
        else
            stringBuilder.append("0");
        ...
        c = ss.substring(5,6);//estado de lampara
        if(c.equals("1"))
            stringBuilder.append("1");
        else
            stringBuilder.append("0");
    ...
    private static String getActuador(String s){
        ...
        if(c.equals("1"))
            stringBuilder.append("Salida Unidad condensadora: Encendido\n");
        else
            stringBuilder.append("Salida Unidad condensadora: Apagado\n");
        ...
        if(c.equals("1"))
            stringBuilder.append("Salida Cerradura: Encendido\n");
        else
            stringBuilder.append("Salida Cerradura: Apagado\n");
            ...
            if (c.equals("1")){
                switch (i)  {
                    ...
                    case 1:
                        ...
                        break;
                    case 0:
                        stb.append( " - Falla sensor ambiental o condensador abierto");
                        break;
                }
            }
        }
            ...
            if (c.equals("1")){
                switch (i)  {
                    ...
                    case 3:
                        ...
                        break;
                    case 2:
                        stb.append( " - Fall sensor ambiental o condensador en corto");
                        break;
                    case 1:
    ...
    public static List<String> hexToAsciiWifi(List<String> hex) {
        //dos ssid para versión WIFI CEO
        arrayListInfo.clear();
        ...
        StringBuilder output = new StringBuilder();
        Log.d("YYY",":"+hexStr);
        int j=0;//identificar el dato, SSID 0, IP=1, MAC_STA 2, IOT_CORE 3, SSID2 4, VER 5, MQTT
        //estado completo
        for (int i = 0; i < hexStr.length(); i+=2) {
    ...
    public static List<String> hexToAsciiWifiRutaFria(List<String> hex) {
        //un solo ssid para este equipo
        arrayListInfo.clear();
        StringBuilder hexStrr = new StringBuilder();
        for (int i = 0; i<hex.size(); i++){
            hexStrr.append(hex.get(i));
        ...
        StringBuilder output = new StringBuilder();
        Log.d("YYY",":"+hexStr);
        int j=0;//identificar el dato, SSID 0, IP=1, MAC_STA 2, IOT_CORE 3
        //estado completo
        for (int i = 0; i < hexStr.length(); i+=2) {
            String strSeparacion = hexStr.substring(i, i+4);
            Log.d("opo",":"+output.length());
    ...
    public static String checkChecksum(String data){
        ...
        for (int p=0; p<data.length()-9 ; p+=2) {
            checksumTotal = checksumTotal + getDecimal(data.substring(p, p + 2));
        }
    ...
    public static  String getFwVersionFromHex(String byte1, String byte2){
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(getDecimal(byte1));
        stringBuilder.append(".");
        //Log.d("VERSION",":"+data.get(3).substring(2));
        int version = getDecimal(byte2);
        if (version<10){
            stringBuilder.append("0"+String.valueOf(version));
        }else{
            stringBuilder.append(getDecimal(byte2));
        }
